...
- **Pages**: `index.html` (home com hero/carrossel/countdown), `sobre.html` (sobre o evento), `palestrantes.html`, `programacao.html`, `noticias.html` (listagem), `noticia.html` (detalhe), `contato.html`.
...
- **JS**: jQuery 1.12.4 + plugins (Slick, SlickNav, WOW.js, Magnific Popup, CounterUp). Homepage e sobre usam Magnific Popup para álbum de fotos com rotação sequencial. Homepage usa Swiper (CDN) com countdown inline.
...
- **Footer unificado**: Todas as páginas compartilham o mesmo footer com informações do evento (editável via CMS: texto, local, data, redes sociais).
- **Imagens**: subdiretórios em português — `assets/img/destaque/`, `assets/img/galeria/`, `assets/img/logos/`, `assets/img/elementos/`.

## CMS (Plone)

Todas as seções de conteúdo são moduladas para integração com Plone:
- **Notícias**: listagem em `noticias.html`, detalhe em `noticia.html`. Sem categorias nem comentários.
...
- **Sobre o Evento**: fonte única no CMS, reflete em `index.html` (resumo) e `sobre.html` (completo).
